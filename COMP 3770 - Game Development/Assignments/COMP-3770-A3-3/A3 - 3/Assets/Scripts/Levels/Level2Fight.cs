...
public class Level2Fight : MonoBehaviour
{
    ...
    int dmg, sHeal, bHeal, heal2, wDmg, rDmg, dDmg, mDmg, pDmg;
    bool dead, xHeal;
    ...
    {
        string path = Application.dataPath + "/CSV/Level2.csv";
    ...
    bool PlayerAttack()
    {
        ...
        if (warrior.currentHP <= 1500)
        {
            heal2 = priest.freeHeal();
            warrior.currentHP += heal2;
            xHeal = true;
        }
        else
            xHeal = false;
        ...
        if (xHeal)
            pAction.text += " Heal Warrior an extra " + heal2 + " HP.";
    ...
    bool BossAttack()
    {
        ...
        bAction.text = "Action: Dealing damage. " + wDmg + " to War, " + rDmg + " to Rge, " + dDmg + " to Dru, " + mDmg + " to Mge, " + pDmg + " to Pst.";
